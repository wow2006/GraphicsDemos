...
auto main(int argc, char *argv[]) -> int {
  ...
  glm::vec4 triangleColor = {1.F, 0.F, 0.F, 1.F};
  ...
  while(bRunning) {
    ...
    while(SDL_PollEvent(&event) > 0) {
      ...
      case SDL_MOUSEMOTION: {
        auto mouseMotionEvent = event.motion;
        triangleColor.x = mouseMotionEvent.x / static_cast<float>(width);
        triangleColor.y = mouseMotionEvent.y / static_cast<float>(height);
      } break;
      }
    }
    ...
    {
      glUniform4fv(0, 1, reinterpret_cast<float*>(&triangleColor));
