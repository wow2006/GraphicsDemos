...
auto main(int argc, char *argv[]) -> int {
  ...
  glVertexArrayVertexBuffer(vao, binding, vbo, 0, sizeof(float) * 8);

  glEnableVertexArrayAttrib(vao,  positionAttribute);
  glVertexArrayAttribFormat(vao,  positionAttribute, 4, GL_FLOAT, GL_FALSE, 0);
  glVertexArrayAttribBinding(vao, positionAttribute, binding);

  glEnableVertexArrayAttrib(vao,  colorAttribute);
  glVertexArrayAttribFormat(vao, colorAttribute, 4, GL_FLOAT, GL_FALSE, sizeof(float) * 4);
  glVertexArrayAttribBinding(vao, colorAttribute, binding);
