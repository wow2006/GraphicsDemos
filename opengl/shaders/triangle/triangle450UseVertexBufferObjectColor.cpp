...
public:
  ...
  void createBuffers() {
    ...
    constexpr std::array<float, 12> cPositions = {
       0.0F, 1.0F, 0.0F, 1.0F,
       1.0F,-1.0F, 0.0F, 1.0F,
      -1.0F,-1.0F, 0.0F, 1.0F
    };

    constexpr std::array<float, 12> cColors = {
      1.0F, 0.0F, 0.0F, 1.0F,
      0.0F, 1.0F, 0.0F, 1.0F,
      0.0F, 0.0F, 1.0F, 1.0F
    };

    glCreateBuffers(2, mVBOs); // NOLINT
    glNamedBufferStorage(mVBOs[0], sizeof(float) * cPositions.size(), cPositions.data(), GL_DYNAMIC_STORAGE_BIT);
    glNamedBufferStorage(mVBOs[1], sizeof(float) * cColors.size(),    cColors.data(),    GL_DYNAMIC_STORAGE_BIT);
    ...
    glVertexArrayVertexBuffer(mVAO,  positionBinding, mVBOs[0], 0, sizeof(float) * 4);
    ...
    glVertexArrayVertexBuffer(mVAO,  colorBinding, mVBOs[1], 0, sizeof(float) * 4);
    glVertexArrayAttribFormat(mVAO,  colorAttrib,  4, GL_FLOAT, GL_FALSE, 0);
